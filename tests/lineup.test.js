import{lineupSnapshot,moveOrSwap,resetToDraftedPositions,restoreSnapshot,validateDraftArrangement,validateLineup,isLineupEditable}from'../js/lineup.js?v=20260721-two-click-1';import{calculateTeamRatings}from'../js/ratings.js?v=20260721-two-click-1';import{draftRosterView,playerDraftView}from'../js/draft-board.js?v=20260721-two-click-1';import{validatePlayerPositions}from'../js/data-validator.js?v=20260721-two-click-1';
...
export const lineupTests=[
 ...
 ['Draft sidebar exposes draggable players and accessible move controls',()=>{const html=draftRosterView({roster:makeRoster().slice(0,2)},null,'SF');return html.includes('draggable="true"')&&html.includes('aria-label="Move Alpha Guard to position"')&&html.includes('draft-position-select')&&html.includes('draft-move-button')}],
 ['Move selector includes only stored eligible positions',()=>{const html=draftRosterView({roster:makeRoster().slice(0,1)},null,'SG');return html.includes('<option value="PG"')&&html.includes('<option value="SG"')&&!html.includes('<option value="C"')}],
 ['Keyboard and touch use the same native select and button action',()=>{const html=draftRosterView({roster:makeRoster().slice(0,1)},null,'SG');return html.includes('<select')&&html.includes('<button type="button"')&&html.includes('aria-live="polite"')}],
